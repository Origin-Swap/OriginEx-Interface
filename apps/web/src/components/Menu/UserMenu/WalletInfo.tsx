...
const WalletInfo: React.FC<WalletInfoProps> = ({ hasLowNativeBalance, onDismiss }) => {
  ...
  const isBSC = chainId === ChainId.VANAR_TESTNET
  const bnbBalance = useBalance({ addressOrName: account, chainId: ChainId.VANAR_TESTNET })
  const nativeBalance = useBalance({ addressOrName: account, enabled: !isBSC })
  const native = useNativeCurrency()
  const wNativeToken = !isBSC ? WNATIVE[chainId] : null
  const wBNBToken = WNATIVE[ChainId.VANAR_TESTNET]
      ...
      <Box mb="24px">
        <Flex justifyContent="space-between" alignItems="center" mb="8px">
          <Flex bg={COLORS.BNB} borderRadius="16px" pl="4px" pr="8px" py="2px">
            <ChainLogo chainId={ChainId.VANAR_TESTNET} />
            <Text color="white" ml="4px">
              Linea
            </Text>
          </Flex>
          <LinkExternal href={getBlockExploreLink(account, 'address', ChainId.VANAR_TESTNET)}>
            {getBlockExploreName(ChainId.VANAR_TESTNET)}
          </LinkExternal>
        </Flex>
        <Flex alignItems="center" justifyContent="space-between">
          <Text color="textSubtle">VG {t('Balance')}</Text>
        ...
        {wBNBBalance.gt(0) && (
          <Flex alignItems="center" justifyContent="space-between">
            <Text color="textSubtle">WVG {t('Balance')}</Text>
        ...
        <Flex alignItems="center" justifyContent="space-between">
          <Text color="textSubtle">{t('ORIX Balance')}</Text>
