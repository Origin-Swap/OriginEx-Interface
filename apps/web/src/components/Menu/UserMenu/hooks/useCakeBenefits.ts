...
const useCakeBenefits = () => {
  ...
  const currentBscBlock = useChainCurrentBlock(ChainId.VANAR_TESTNET)
  ...
  const { data, status } = useSWR(account && currentBscBlock && ['cakeBenefits', account], async () => {
    ...
    if (lockPosition === VaultPosition.Locked) {
      ...
      const poolAddresses = eligiblePools.map(({ contractAddress }) => getAddress(contractAddress, ChainId.VANAR_TESTNET))
      ...
      const [cakeVaultBalance, total] = await getScores(
        ...
        ChainId.VANAR_TESTNET.toString(),
        [account],
        currentBscBlock,
      )
      vCake = {
