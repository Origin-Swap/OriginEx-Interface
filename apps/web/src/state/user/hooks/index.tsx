...
export function useGasPrice(chainIdOverride?: number): string {
  ...
  const { data } = useFeeData({
    chainId,
    enabled: chainId !== ChainId.VANAR_TESTNET && chainId !== ChainId.VANAR_TESTNET,
    watch: true,
  })
  if (chainId === ChainId.VANAR_TESTNET) {
...
export function toV2LiquidityToken([tokenA, tokenB]: [ERC20Token, ERC20Token]): ERC20Token {
  return new ERC20Token(tokenA.chainId, Pair.getAddress(tokenA, tokenB), 18, 'OriginEx LPs', 'OEX-LP')
}
