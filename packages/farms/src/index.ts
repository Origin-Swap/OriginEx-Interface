...
const supportedChainId = [ChainId.VANAR_TESTNET]
...
export function createFarmFetcher(multicallv2: MultiCallV2) {
  const fetchFarms = async (
    ...
    const masterChefAddress = isTestnet ? masterChefAddresses[ChainId.BSC] : masterChefAddresses[ChainId.VANAR_TESTNET]
  ...
  return {
    ...
    supportedChainId,
    isTestnet: (chainId: number) => ![ChainId.VANAR_TESTNET, ChainId.ETHEREUM].includes(chainId),
  }
}
