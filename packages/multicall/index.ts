...
export const multicallAddresses = {
  ...
  78600: '0xBFD2d7354Ef7BB80140388d00D7eD223e1251b65',
}
...
export function createMulticall<TProvider extends Provider>(
  ...
  const multicall: MultiCall = async (abi: any[], calls: Call[], chainId = ChainId.VANAR_TESTNET) => {
  ...
  const multicallv3 = async ({ calls, chainId = ChainId.VANAR_TESTNET, allowFailure, overrides }: MulticallV3Params) => {
    const multi = getMulticallContract(chainId, provider({ chainId }))
    if (!multi) throw new Error(`Multicall Provider missing for ${chainId}`)
